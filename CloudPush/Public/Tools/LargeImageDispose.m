...
-(void)dealloc {
    // [SDImageCache sharedImageCache].config.SDImageCacheOldShouldDecompressImages = SDImageCacheOldShouldDecompressImages;
    //config.shouldDecompressImages = SDImageCacheOldShouldDecompressImages;
   // SDImageCacheConfig* config =
    SDWebImageDownloader *downloder = [SDWebImageDownloader sharedDownloader];
    downloder.shouldDecompressImages = SDImagedownloderOldShouldDecompressImages;
}

//改变了图片的渲染方式，利用GPU 进行渲染，有效降低内存，不改变图片的质量，亲测加载24M图片so easy，只占20多内存
//不能用于加载太小的图片，只能用于加载大图（大概1M以上）
//导致加载大图内存暴涨的原因是对大图的解压缩加载。
//在SDWebImage中大图禁用解压缩，可以防止内存暴涨：
//只有iPhone7，7p,SE,有用，其他机型不起作用，和系统版本没有关系
