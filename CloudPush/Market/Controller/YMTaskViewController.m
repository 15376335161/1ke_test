...
-(void)requestDataWithPage:(NSInteger)page isLoading:(BOOL)isLoading type:(NSString* )type param:(NSMutableDictionary* )oldParam{
  ...
  //  NSMutableDictionary* param =
    ...
    YMWeakSelf;
    [[HttpManger sharedInstance]getHTTPReqAPI:urlStr params:param view:self.view loading:isLoading tableView:self.tableView completionHandler:^(id task, id responseObject, NSError *error) {
        if (_page > 0 ) {
