...
- (void)setUpTitlesView{
    ...
    tagScrollView.y = 1;
    tagScrollView.contentSize = CGSizeMake((SCREEN_WIDTH / 3) * self.childViewControllers.count, TitleViewHeigh);
    ...
    NSArray* tagsArr = @[@"审核中",@"审核通过",@"审核不通过"];
    for (int i = 0; i < tagsArr.count; i ++) {
        ...
        if (i == 0) {
            ...
            //根据button内部的文字宽度计算尺寸
            self.indicatorView.width   =  btn.titleLabel.width;//SCREEN_WIDTH / 3;//
...
- (void)titleClick:(UIButton *)button{
   ...
   // [button setTitleColor:NavBarTintColor forState:UIControlStateSelected];
    ...
    if (button.tag == 3) {
        offset_tag.x = (SCREEN_WIDTH / 3) * button.tag;
